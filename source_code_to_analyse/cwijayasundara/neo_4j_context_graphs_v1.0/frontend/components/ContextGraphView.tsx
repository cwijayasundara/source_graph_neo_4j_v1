...
function extractNodesAndRels(results: Record<string, unknown>[]): InternalGraphData {
  ...
  function processValue(value: unknown, depth = 0) {
    ...
    // Relationship: support both backend graph payloads and /expand payloads.
    const startNodeId = v.startNodeElementId ?? v.startNodeId ?? v.startNode;
    const endNodeId = v.endNodeElementId ?? v.endNodeId ?? v.endNode;
    if (v.type && startNodeId && endNodeId) {
...
function truncateCaption(value: string, maxLength = 34): string {
...
function getNodeCaption(node: GraphNode): string {
  const candidates = [
    ...
    node.properties.merchant,
    node.properties.raw_description,
    node.properties.id,
  ...
  for (const candidate of candidates) {
    if (typeof candidate === "string" && candidate.trim()) {
      return truncateCaption(candidate.trim());
    }
  }
...
function hasExternalGraphRecords(graphData?: GraphData | null): boolean {
  return Boolean(
    graphData?.results?.length ||
...
export function ContextGraphView({ externalGraphData, onAskAbout, subtitle }: ContextGraphViewProps) {
  ...
  const schemaLoadStartedRef = useRef(false);

  // Load schema only when mounted without supplied graph data. Story/preset
  // updates are handled by the externalGraphData effect below.
  useEffect(() => {
    if (!schemaLoadStartedRef.current && !hasExternalGraphRecords(externalGraphData)) {
      schemaLoadStartedRef.current = true;
      loadSchema();
    }
  });
  ...
  useEffect(() => {
    const records = externalGraphData?.results?.length
      ? externalGraphData.results
      : externalGraphData?.nodes?.length || externalGraphData?.relationships?.length
        ? [externalGraphData as Record<string, unknown>]
        : [];

    if (records.length) {
      const data = extractNodesAndRels(records);
  ...
  const nvlData = useMemo(() => {
    ...
    const relationships: NvlRelationship[] = graphData.relationships.map((rel) => {
      ...
      return {
        ...
        to: rel.endNodeId,
        caption: isSelected || isSchemaView ? rel.type : undefined,
        ...
        <Box>
          ...
          <Text fontSize="xs" color="gray.500">
            {isSchemaView
              ...
              : subtitle || "Financial Services entity relationships"}
          </Text>
        </Box>
        {!isSchemaView && (
          <IconButton
