...
## Task 1: Shared model resolver; route BRD through it
...
_ALL = ["BRD_AGENT_MODEL", "ENRICHMENT_MODEL", "ASK_MODEL", "CODE_GRAPH_LLM_MODEL"]
...
# Hardcoded final fallbacks (Claude tiers): cheap Haiku for high-volume roles,
# Sonnet for the heavier BRD synthesis/judge.
_DEFAULTS = {
...
In `src/code_context_graph/brd/pipeline.py`, find the line (currently ~86):
...
Add the import near the other agent imports inside the function (this module uses lazy in-function imports of `agent.*` to avoid a circular import — follow that existing pattern; put `from code_context_graph.agent.models import resolve_model` inside `generate_brd_graph_sync` alongside the existing lazy imports, OR at top-of-file only if it does not reintroduce the cycle — `agent.models` imports only `os`, so a top-level import is safe here). Verify the BRD tests still pass:
...
In `.env.example`, change the BRD agent section so the global var is documented as the Claude default and the per-role overrides are shown. Replace the existing `CODE_GRAPH_LLM_MODEL=...` line and the `BRD_AGENT_MODEL` line with:

```bash
# Claude model selection (single source of truth). Per-role overrides win over the
# global; leave overrides unset to use the global. All must be Claude model ids.
CODE_GRAPH_LLM_MODEL=claude-sonnet-4-6     # global default for every LLM path
BRD_AGENT_MODEL=                            # optional override for BRD synthesis/judge
ENRICHMENT_MODEL=                           # optional override (defaults to Haiku)
ASK_MODEL=                                  # optional override (defaults to Haiku)
```

Then update the LIVE `.env` (gitignored — do NOT commit it): change its `CODE_GRAPH_LLM_MODEL=gemini-3.5-flash` line to `CODE_GRAPH_LLM_MODEL=claude-sonnet-4-6`. Use the Edit tool on `.env`. If `.env` is absent or the line is missing, skip and note it.
...
git commit -m "feat(agent): single resolve_model() selector; route BRD through it"
```
